...
**The prototype** (`app.py`): sign in as a judge or as the court master; the sidebar holds only the Samay mark, the name and Sign out. The court master runs several judges from one dashboard: a card per judge with docket size, listings a day, reach, disposals and leave; a Docket files tab to upload each judge's docket (Excel or CSV, checked, with the required columns and an example of how the rows look); Run a day to call each matter and record the outcome, with the next date following by rule; New complaint for registry scrutiny at e-filing. The judge sees Today (a timeline of the two sittings, the list with time, listing number, score, advocate and why today; remove, restore, approve), Week (five days side by side with each day's hearings, holidays and leave), Cases (every case with score, status, flags and history), Priority (the five weights, adjustable, age never below 20, re-scoring live), Insight (the five measures against today's rules, success by listing number, the cumulative curve), and How Samay decides (the score, the model, the day-building steps, and every parameter with its value for this judge). Pages are fixed to the screen; tables scroll in their own frames. `docs/Samay_docket_Justice_Sehgal.xlsx` is your 100-case roster as a sheet for the trial.
